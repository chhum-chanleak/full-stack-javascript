...
it("should call add with correct arguments", () => {
  const mockAdd = add as jest.Mock; // In Jest, the "jest.Mock" type is used to define the type of a mocked function. It provides you with
  ...
  // Call add(2, 3)
  const addResult = add(2, 3); // // The call to add(2, 3) is mocked because the imported function add is replaced with a mock during the test setup. The real implementation of add is not executed; instead, the mocked behavior is used.
...
it("should call subtract with correct arguments", () => {
  ...
  const mockSubtract = subtract as jest.Mock // In Jest, the "jest.Mock" type is used to define the type of a mocked function. It provides you with
  ...
  const subtractResult = subtract(3, 1); // The call to subtract(3, 1) is mocked because the imported function subtract is replaced with a mock during the test setup. The real implementation of subtract is not executed; instead, the mocked behavior is used.
...
test("a Logger class", () => {
  ...
  const MockLogger = Logger as jest.Mock; // Logger can be asserted as jest.Mock type only when it had been already mocked in jest.mock() implementation
...
test("a Calculator class", () => {
   // Type assertion to treat(convert) Logger as a jest mock
  const MockCalculator = Calculator as jest.Mock; // Calculator can be asserted as jest.Mock type only when it had been already mocked in jest.mock() implementation
  // Instantiating the Mocked Class
